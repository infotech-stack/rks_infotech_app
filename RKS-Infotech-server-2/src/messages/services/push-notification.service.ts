...
//     this.subscriptions.push({ subscription, message });
//   }
...
//     for (const { subscription, message } of this.subscriptions) {
//         const sender= await dbConnection.query(`
//            SELECT employee_name FROM task_management.employee_register where empId=${message.sender_id} ;
//             `);
//         const reciever= await dbConnection.query(`
//            SELECT employee_name FROM task_management.employee_register where empId=${message.receiver_id};
//             `);
//       const title = `Message from ${sender[0].employee_name} Message to ${reciever[0].employee_name}`;
