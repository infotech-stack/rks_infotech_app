...
export class EmployeeRegisterService {
  ...
  async logoutMethod(empId: number): Promise<ResponseInterface> {
    try {
      ...
      // const formattedLoginDateTime = logoutDateTime.toISOString().slice(0, 19).replace('T', ' ');
  ...
  async registerEmployee(employeeDetails: InsertEmployeeInterface): Promise<ResponseInterface> {
    try {
      ...
      // Fetch role_id from employee_roles table based on role names
  ...
  console.log(roleIds,'roleid');
      ...
      // Fetch access_id from employee_access table based on access names
      const accessIds: number[] = [];
      for (const accessName of employeeDetails.employee_access) {
        const accessResult = await dbConnection.query(
          `
            SELECT access_id
...
for (const accessId of accessIds) {
  ...
  //EMPLOYEE ATTENDANCE
  async employeeAttendance(current_page?:number,page_size?:number,all?:boolean): Promise<ResponseInterface> {
    // try {
    ...
    //       `);
    //       const totalRecords = await dbConnection.query(`
    //         SELECT COUNT(*) AS total 
    //         FROM 
    //           task_management.employee_register er
    //         JOIN 
    ...
    try {
      ...
      if (all) {
        ...
        const totalRecordsResult = await dbConnection.query(`
        ...
        // Fetch paginated records with direct values
        data = await dbConnection.query(`
          SELECT 
            er.empId,
            er.employee_name,
